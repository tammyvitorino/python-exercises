...
for i in range(n):
    ...
    if operation == '+':
        num = int(expression[0])*int(expression[6]) + int(expression[4])*int(expression[2])
        den = int(expression[2])*int(expression[6])
    ...
    if operation == '*':
        num = int(expression[0])*int(expression[4])
        den = int(expression[2])*int(expression[6])

    if operation == '/':
        num = int(expression[0])*int(expression[6])
        den = int(expression[2])*int(expression[4])

    mdc = MDC(num, den) 
    simp = str(int(num / mdc)) + '/' + str(int(den / mdc))
    results.append(str(num) + '/' + str(den) + ' = ' + simp)
